...
} from './AccountOnFileAttribute';

export class AccountOnFile {
  private readonly attributeByKey: Record<
    string,
    AccountOnFileAttribute | undefined
  >;
  ...
  getRequiredAttributes(): AccountOnFileAttribute[] {
    return this.attributes.filter(
      (attribute) =>
        attribute.status === AccountOnFileAttributeStatus.MUST_WRITE
    );
  }

  getWritableAttributes(): AccountOnFileAttribute[] {
    return this.attributes.filter(
      (attribute) => attribute.status !== AccountOnFileAttributeStatus.READ_ONLY
    );
  }
  ...
  isWritable(fieldId: string): boolean {
    return (
      this.attributeByKey[fieldId]?.status !==
      AccountOnFileAttributeStatus.READ_ONLY
    );
  }
}
