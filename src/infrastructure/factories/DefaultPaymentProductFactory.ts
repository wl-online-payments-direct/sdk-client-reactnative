...
export class DefaultPaymentProductFactory implements PaymentProductFactory {
  createBasicPaymentProducts(dto: BasicPaymentProductsDto) {
    const products = dto.paymentProducts.map((product) =>
      this.createBasicPaymentProduct(product)
    );
  ...
  createAccountOnFile(dto: AccountOnFileDto): AccountOnFile {
    ...
    const mask = dto.displayHints?.labelTemplate?.find(
      (t) => t.attributeKey === 'alias'
    )?.mask;
    ...
    return new AccountOnFile(
      ...
      dto.attributes.map(
        (a) => new AccountOnFileAttribute(a.key, a.value, a.status)
      )
    );
  }
  ...
  createDisplayHintsForField(
    dto?: ProductFieldDisplayHintsDto
  ): ProductFieldDisplayHints {
  ...
  createDataRestrictions(dto: DataRestrictionsDto): DataRestrictions {
    return new DataRestrictions(
      ...
      new ValidationRuleFactory().createRules(dto.validators)
    );
  }
}
