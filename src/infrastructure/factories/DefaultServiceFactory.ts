...
import type { ApplePay } from '../../services/models/ApplePay';
import { Encryptor } from '../encryption/Encryptor';
import type { EncryptionProvider } from '../encryption/EncryptionProvider';
import type { DeviceInformationProvider } from '../interfaces/DeviceInformationProvider';
import { DefaultDeviceInformationProvider } from '../DefaultDeviceInformationProvider';
...
export class DefaultServiceFactory implements ServiceFactory {
  ...
  constructor(props: ServiceFactoryProps) {
    ...
    this.applePay = props.applePay ?? {
      async isApplePayAvailable(): Promise<boolean> {
        return false;
      },
    };

    this.deviceInformationProvider =
      props.deviceInformationProvider ??
      new DefaultDeviceInformationProvider(props.configuration);
    ...
    this.paymentProductFactory =
      props.paymentProductFactory ?? new DefaultPaymentProductFactory();

    this.encryptionProvider =
      props.encryptionProvider ??
      new Encryptor({
        clientSessionId: props.sessionData.clientSessionId,
      });
    ...
    this.clientService =
      props.clientService ??
      new DefaultClientService(this.getCacheManager(), this.apiClient);

    this.paymentProductService =
      props.paymentProductService ??
      new DefaultPaymentProductService(
        this.getCacheManager(),
        this.apiClient,
        this.paymentProductFactory,
        this.applePay
