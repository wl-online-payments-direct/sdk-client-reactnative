...
describe('createCacheKeyFromContext', () => {
  ...
  it('omits undefined and empty-string suffix values', () => {
  ...
  it('includes isRecurring: false in cache key (not dropped)', () => {
    const key = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
  ...
  it('creates different keys for isRecurring: true vs isRecurring: false', () => {
    const keyTrue = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
      context: { ...baseContext, isRecurring: true },
    });
    const keyFalse = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
      context: { ...baseContext, isRecurring: false },
    });
  ...
  it('includes amount: 0 in cache key (not dropped)', () => {
    const key = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
      context: {
        ...baseContext,
  ...
  it('creates different keys for amount: 0 vs amount: 100', () => {
    const keyZero = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
      context: {
        ...baseContext,
        amountOfMoney: { amount: 0, currencyCode: 'EUR' },
    ...
    const keyHundred = cacheManager.createCacheKeyFromContext({
      prefix: 'payment',
      context: {
        ...baseContext,
    ...
    expect(keyZero).not.toBe(keyHundred);
  });
