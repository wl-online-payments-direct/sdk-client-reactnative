...
describe('encrypt', () => {
  ...
  const encryptPaymentRequestAndValidateHeader = (
    paymentRequest: PaymentRequest
  ) => {
    const encryptedString = encryptor.encrypt(
      publicKeyResponse,
      paymentRequest
    );
    ...
    if (!headerPart) {
      throw new Error(
        'Expected encrypted payment request to contain a header.'
      );
    }
  ...
  it('includes accountOnFileId in payload when account on file is set', () => {
    ...
    const joseEncryptSpy = vi
      .spyOn(JOSEEncryptor, 'encrypt')
      .mockReturnValue('a.b.c.d.e');
    encryptor.encrypt(publicKeyResponse, request);

    expect(joseEncryptSpy).toHaveBeenCalledTimes(1);

    expect(joseEncryptSpy.mock.calls[0]![0]).toMatchObject({
      accountOnFileId: 'aof-123',
    });
  });
...
describe('encryptTokenRequest', () => {
  ...
  const encryptTokenRequestAndValidateHeader = (
    creditCardTokenRequest: CreditCardTokenRequest
  ) => {
    const encryptedString = encryptor.encryptTokenRequest(
      publicKeyResponse,
      creditCardTokenRequest
    );
    ...
    if (!headerPart) {
      throw new Error('Expected encrypted token request to contain a header.');
    }
  ...
  it('throws EncryptionError when payment product id is not set', () => {
    const action = () =>
      encryptor.encryptTokenRequest(
        publicKeyResponse,
        new CreditCardTokenRequest()
      );
...
describe('nonce format', () => {
  it('nonce in encrypted payload matches /^[0-9a-f]{32}$/ (16 hex-encoded random bytes)', () => {
    ...
    const joseEncryptSpy = vi
      ...
      .mockImplementation((payload) => {
        capturedNonce = (payload as any).nonce;
        return 'a.b.c.d.e';
      });
    ...
    expect(joseEncryptSpy).toHaveBeenCalledTimes(1);
    expect(capturedNonce).toMatch(/^[0-9a-f]{32}$/);
  });
});
