...
describe('ValidationRuleFactory', () => {
  ...
  describe('createRules', () => {
    const createRulesWithInvalidValidators = (validators: unknown) =>
      factory.createRules(validators as never);
    ...
    it('creates a ValidationRuleRegularExpression with the correct regular expression', () => {
      const rules = factory.createRules({
        regularExpression: { regularExpression: '^\\d+$' },
      });
      ...
      if (!rule) {
        throw new Error('Expected one regular expression validation rule.');
      }

      expect(rule).toBeInstanceOf(ValidationRuleRegularExpression);
      expect(rule.validate('123').valid).toBe(true);
    ...
    it('creates a ValidationRuleFixedList with the correct allowed values', () => {
      ...
      if (!rule) {
        throw new Error('Expected one fixed list validation rule.');
      }

      expect(rule).toBeInstanceOf(ValidationRuleFixedList);
      expect(rule.validate('a').valid).toBe(true);
      expect(rule.validate('c').valid).toBe(false);
    });
    ...
    it('skips the length rule when minLength or maxLength is missing', () => {
      const rulesWithoutMin = createRulesWithInvalidValidators({
        length: { maxLength: 10 },
      });
      const rulesWithoutMax = createRulesWithInvalidValidators({
        length: { minLength: 2 },
      });
