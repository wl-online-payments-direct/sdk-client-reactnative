...
beforeEach(() => {
  ...
  accountOnFile = new DefaultPaymentProductFactory().createAccountOnFile(
    accountOnFileJson
  );
...
describe('Test values that are `READ_ONLY`', () => {
  ...
  it('should remove field values after setting accountOnFile, and return `undefined` if `READ_ONLY`', () => {
    const localPaymentRequest = new PaymentRequest(paymentProduct);
    expect(localPaymentRequest.getAccountOnFile()).toBe(undefined);

    localPaymentRequest.getField('cardNumber').setValue('1111111111');
    localPaymentRequest.setAccountOnFile(accountOnFile);

    expect(() =>
      localPaymentRequest.getField('cardNumber').setValue('2222222222')
    ).toThrowError();
    expect(localPaymentRequest.getField('cardNumber').getValue()).toBe(
      undefined
    );
    expect(paymentRequest.getAccountOnFile()).not.toBe(undefined);
    expect(paymentRequest.getAccountOnFile()?.id).toBe('1234');
...
describe('validate', () => {
  ...
  it('should return error for `cvv` when it is `MUST_WRITE` in accountOnFile and user provides no value with AOF', () => {
    const accountOnFileWithMustWriteCvv =
      new DefaultPaymentProductFactory().createAccountOnFile(
        accountOnFileWithMustWriteCvvJson
      );
  ...
  it('should pass validation when `cardholderName` is `CAN_WRITE` and user does not provide value it should not validate it', () => {
    const accountOnFileWithCanWriteCardholderName =
      new DefaultPaymentProductFactory().createAccountOnFile(
        accountOnFileWithCardHolderNameCanWriteJson
      );
...
describe('encrypt', () => {
  ...
  beforeEach(() => {
    const mockDeviceInformationProvider: DeviceInformationProvider = {
      getMetadata: () => ({
