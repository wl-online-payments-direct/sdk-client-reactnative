...
type SessionRequest = Parameters<
  typeof defaultSdkClient.sessions.createSession
>[1];
type PaymentContext = Parameters<
  typeof defaultSdkClient.sessions.createSession
>[2];
type CreatePaymentRequest = Parameters<
  typeof defaultSdkClient.payments.createPayment
>[1];
...
export async function getSessionFromSdk({
  ...
  try {
    const sdkResponse = await client.sessions.createSession(
      merchantId,
      sessionRequest,
      paymentContext
    );
