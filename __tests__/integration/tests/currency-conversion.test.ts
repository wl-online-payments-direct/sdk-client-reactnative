...
import {
  ...
  withNoCurrencyConversionErrorResponseJson,
} from '../__fixtures__/currency-conversion';
...
describe.skip('GetCurrencyConversionQuote', () => {
  ...
  it('GetCurrencyConversionQuote Returns proposal with card and payment product id', async () => {
    ...
    const result = await session.getCurrencyConversionQuote(
      amountOfMoney,
      partialCard
    );

    expect(result.proposal?.baseAmount).toStrictEqual(
      withCurrencyConversion.proposal.baseAmount
    );
  });
  ...
  it('GetCurrencyConversionQuote Throws error with token source', async () => {
    const cardWithoutCurrencyConversionToken = getEnvVar(
      'VITE_CARD_TOKEN_WITHOUT_SURCHARGE_CURRENCY_CONVERSION'
    );
  ...
  const expectCurrencyConversionNotFound = async (
    cardOrToken: PartialCard | string
  ) => {
    const expectedErrorResponse =
      withNoCurrencyConversionErrorResponseJson.errors[0]!;

    await expect(() =>
      session.getCurrencyConversionQuote(amountOfMoney, cardOrToken)
    ).rejects.toThrow(
